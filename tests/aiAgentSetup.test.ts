...
test("keeps the real upload error and targets setup commands at the current environment", () => {
  ...
  assert.match(commands.macInstall, /AMBRIUM_API<\/key><string>https:\/\/staging\.example/)
  assert.match(commands.linuxInstall, /Environment=AMBRIUM_API=https:\/\/staging\.example/)
